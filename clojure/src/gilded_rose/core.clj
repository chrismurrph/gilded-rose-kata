...
(defn start-inventory [include-enhance?]
  (cond-> [(item "+5 Dexterity Vest" 10 20)
           (item "Aged Brie" 2 0)
           (item "Elixir of the Mongoose" 5 7)
           (item "Sulfuras, Hand Of Ragnaros" 0 80)
           (item "Backstage passes to a TAFKAL80ETC concert" 15 20)]
          include-enhance? (conj (item "Conjured" 10 10))))

(defn update-current-inventory
  ([update-quality-f inventory]
   (update-quality-f inventory))
  ([]
   (assert false "Always want to supply the inventory")
   (update-current-inventory update-quality (start-inventory false))))
